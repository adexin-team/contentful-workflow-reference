...
import { prepareProductionPromotion } from "./lib/promotion.js"
...
export async function main(): Promise<void> {
  ...
  const candidate = await prepareProductionPromotion({ activation: {
  ...
  console.log(`Prepared read-only candidate ${candidate.candidateDigest}.`)
  console.log(`Separate confirmation required: PROMOTE:${candidate.candidateDigest}:TO:${project.productionEnvironment}`)
}
